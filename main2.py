...
# ================================================================
# Features:
# - Live webcam preview in Tkinter (no web server)
# - Attentive / Distracted / Sleeping detection using MediaPipe FaceMesh
#   - EAR (eye aspect ratio) for sleeping detection
#   - Head yaw proxy (nose vs. eye-center) for distraction
# - Student counting, per-face status list
# - Engagement score (exponential moving average of attentive ratio)
# - CSV logging (minute cadence while faces are present)
# - PDF report generation from CSV
# - Threaded capture+inference loop (UI remains responsive)
# - Graceful shutdown, camera switching, and extensive error handling
#
# Dependencies:
#   pip install opencv-python mediapipe==0.10.14 numpy pandas fpdf2 pillow
#
# Author: (You)
...
class AppConfig:
    ...
    # Heuristics for attention
    ear_sleep_thresh: float = 0.18        # lower → stricter sleeping detection
    ear_frames_sleep: int = 15            # consecutive frames below EAR to mark sleeping
    yaw_ratio_thresh: float = 0.28        # higher → more tolerant before calling distracted
...
# --------------------------
# Engagement Score (EMA)
# --------------------------

class EngagementTracker:
    """
    Simple EMA over attentive ratio.
    """
...
# --------------------------
# MediaPipe-based Detector
# --------------------------

mp_face_mesh = mp.solutions.face_mesh

# Canonical mesh landmark indices (stable enough for our heuristics)
...
def eye_aspect_ratio(pts: List[Tuple[float, float]]) -> float:
    """
    conventional EAR-like ratio with 6 points:
    (||p2-p6|| + ||p3-p5||) / (2 * ||p1-p4||)
    """
...
class ClassDetector:
    """
    Wraps OpenCV VideoCapture + MediaPipe FaceMesh inference.
    Provides infer(frame) → (infos, totals)
    """
    def __init__(self, cfg: AppConfig):
        self.cfg = cfg
        self.cap = None
        self.mesh = None
        self.sleep_counters: Dict[int, int] = {}  # ephemeral index-based counters
    ...
    def _open_camera(self):
        ...
        self.cap = cv2.VideoCapture(self.cfg.camera_index, cv2.CAP_DSHOW)  # CAP_DSHOW: Windows-friendly
    ...
    def infer(self, frame):
        ...
        Returns:
            infos: list of dicts
                [{'id': int, 'status': 'Attentive'|'Distracted'|'Sleeping',
                  'ear': float, 'yaw_ratio': float, 'nose': (x,y)}]
            totals: (total, attentive, distracted, sleeping)
        """
        h, w = frame.shape[:2]
        ...
        if res.multi_face_landmarks:
            for idx, fl in enumerate(res.multi_face_landmarks):
                ...
                def pick(idxs):
                    return [self._norm2pix(fl.landmark[i], w, h) for i in idxs]

                left = pick(LEFT_EYE)
                right = pick(RIGHT_EYE)
                nose_x, nose_y = self._norm2pix(fl.landmark[NOSE_TIP], w, h)

                # Head yaw proxy: nose deviation from eye center normalized by the eye span
                lx, rx = left[0][0], right[3][0]  # outer corners
                ...
                # Sleep counter per ephemeral face id
                sid = idx
                counter = self.sleep_counters.get(sid, 0)
                if ear < self.cfg.ear_sleep_thresh:
                    counter += 1
                else:
                    counter = 0
        ...
        return infos, (face_count, attentive, distracted, sleeping)
...
# --------------------------
# Threaded Capture Controller
# --------------------------

class CaptureController:
    """
    Manages the camera loop in a background thread.
    Produces frames and stats for the GUI to consume.
    """
    def __init__(self, cfg: AppConfig):
        self.cfg = cfg
    ...
    def switch_camera(self, index: int):
        """
        Safe camera switch. Stops loop briefly, switches, and resumes.
        """
    ...
    def _loop(self):
        while self.running:
            try:
                ...
                infos, totals = self.detector.infer(frame)
                total, attn, dist, sleep = totals
                ratio = (attn / max(1, total)) if total else 0.0
                eng_val = self.eng.update(ratio)

                # draw overlay text (optional light overlay)
                overlay = frame.copy()
                cv2.putText(overlay, f"Total: {total}", (20, 40),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                cv2.putText(overlay, f"Attentive: {attn}", (20, 80),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(overlay, f"Distracted: {dist}", (20, 120),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                cv2.putText(overlay, f"Sleeping: {sleep}", (20, 160),
                ...
                # periodic CSV logging (only if faces present)
                now = time.time()
                ...
                # Avoid crashing; surface in UI via a label
                traceback.print_exc()
                time.sleep(0.1)

    # Snapshot getters for UI thread (avoid holding lock too long)
    def get_snapshot(self):
        with self.lock:
            frame = None if self.latest_frame is None else self.latest_frame.copy()
            infos = list(self.latest_infos)
            return (frame, infos,
                    self.total, self.attentive, self.distracted, self.sleeping, self.engagement)
...
class ClassWatchApp(tk.Tk):
    def __init__(self, cfg: AppConfig):
        ...
        # Create controller after UI (so we can immediately reflect errors)
        try:
    ...
    def _build_ui(self):
        ...
        # Right panel: face table + actions
    ...
    def _apply_settings(self):
        if self._building_ui or self.ctrl is None:
            return
    ...
    def _refresh_ui(self):
        """
        Periodically update the GUI with latest capture info and image.
        """
        try:
            if self.ctrl is not None:
                ...
                try:
                    self.eng_var.set(f"{float(eng):.2f}")
                except Exception:
                    self.eng_var.set(str(eng))
                ...
                if frame is not None:
                    ...
                    self.preview_label.image = disp  # keep ref
        except Exception:
            traceback.print_exc()
